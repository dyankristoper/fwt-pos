...
const BranchVatSettings = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      await Promise.all([
        supabase.from('pos_settings').update({
  ...
  const inputClass = 'w-full h-10 mt-1 px-3 bg-background border-2 border-foreground/10 rounded-lg font-body text-sm text-foreground focus:border-accent focus:outline-none transition-colors';
    ...
    <div className="bg-foreground/[0.03] rounded-xl p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input type="text" value={branch.code} onChange={e => setBranch({ ...branch, code: e.target.value })}
            maxLength={10} className={inputClass} placeholder="QC01" />
        </div>
        <div>
          <label className={labelClass}>Branch Name</label>
          <input type="text" value={branch.name} onChange={e => setBranch({ ...branch, name: e.target.value })}
            maxLength={100} className={inputClass} placeholder="Main Branch" />
        </div>
      </div>

      <div>
        <label className={labelClass}>Legal Name *</label>
        <input type="text" value={branch.legal_name} onChange={e => setBranch({ ...branch, legal_name: e.target.value })}
          maxLength={200} className={inputClass} placeholder="Featherweight Chicken Inc." />
      </div>

      <div>
        <label className={labelClass}>Address</label>
        <input type="text" value={branch.address} onChange={e => setBranch({ ...branch, address: e.target.value })}
          maxLength={300} className={inputClass} placeholder="123 Main St, Quezon City" />
      </div>

      <div>
        <label className={labelClass}>TIN *</label>
        <input type="text" value={branch.tin} onChange={e => setBranch({ ...branch, tin: e.target.value })}
          maxLength={20} className={inputClass} placeholder="000-000-000-000" />
      </div>
      ...
      <div className="flex items-center justify-between pt-1">
        ...
        <button onClick={() => setVatInclusive(!vatInclusive)} className="flex items-center gap-2 active:scale-[0.97] transition-transform">
          <span className="font-display text-xs font-bold text-foreground/60">{vatInclusive ? 'Inclusive' : 'Exclusive'}</span>
          {vatInclusive ? <ToggleRight size={28} className="text-pos-gold-dark" /> : <ToggleLeft size={28} className="text-foreground/30" />}
        </button>
      </div>
