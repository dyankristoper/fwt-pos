...
const DailySummary = ({ summary, onBack }: DailySummaryProps) => {
            ...
            <div className="bg-card rounded-xl border-2 border-foreground/5 overflow-hidden">
              ...
              {summary.orders
                ...
                .map(order => (
                  <div
                    key={order.id}
                    className="grid grid-cols-4 gap-4 p-4 border-t border-foreground/5"
                  >
                    <span className="font-display font-bold text-sm">{order.id}</span>
                    <span className="text-foreground/60 text-sm">
                      {order.timestamp.toLocaleTimeString('en-PH', {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}
                    </span>
                    ...
                    <span className="text-right font-display font-bold text-sm">
...
const PaymentCard = ({
  ...
  <div className="bg-card rounded-xl p-4 border-2 border-foreground/5">
    <div className="flex items-center gap-2 mb-2">
      ...
      <span className="font-display font-semibold text-xs text-foreground/60 uppercase tracking-wide">
        {label}
      </span>
    </div>
